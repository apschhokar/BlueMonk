...
//        private static String filename;
        ...
        private static final String upLoadServerUrl = "http://192.168.0.12:3000/upload";
        private static final String postServerUrl = "http://192.168.2.114:3000";
...
//    private static ConcurrentLinkedQueue<HashMap<String, String>> queue;
    ...
    private static AsyncTask uploadTask = null;
    private static Internals Internals = null;

    private OpLogsSyncAdapter(Context context, String databasName) {
        //Log.d(adaTAG, "OpLogsSyncAdapter() constructor called");
        Internals.filename = "db" + ".log";
        Internals.file = new File(context.getFilesDir(), Internals.filename);
        try {
            if (!Internals.file.exists()) {
                Internals.file.createNewFile();
                if (BmUtilsSingleton.INSTANCE.DEBUG) {
                Log.d(Internals.TAG, "File created " + Internals.filename); }
            }
            Internals.fileOutputStream = new FileOutputStream(Internals.file);
        } catch (Exception e) {
            e.printStackTrace();
        }

        //Used in eaarlier implementations
...
//        new FileWriterTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, fileOutputStream);
        ...
        uploadTask = new FileUploaderTask().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, Internals.fileOutputStream);
    }
        ...
        if (null == adapter) {
            synchronized (OpLogsSyncAdapter.class) {
                if (null == adapter)
                    ...
                    Internals.queue = new ConcurrentLinkedQueue<Integer>();
            }
        }
        return adapter;
    }
...
//            sendDataTask.start();
    ...
    public static void clear(){
        ...
        try{
            ...
            if (null != Internals.fileOutputStream) {
                Internals.fileOutputStream.flush();
                Internals.fileOutputStream.close();
                if (BmUtilsSingleton.INSTANCE.DEBUG) {
                    Log.d(Internals.TAG, "Fileoutputstream closed");
                }
            }
            ...
            Log.e(OpLogsSyncAdapter.class.getSimpleName(),"Error in clear " + e.getMessage());
            e.printStackTrace();
        }
    }

    public void log(String content, String method) {
        try {
            content = String.valueOf(System.nanoTime()) + ":" + content + "\n";
            //fileOutputStream.write(content.getBytes());
            final String data = content;
...
//            queue.add(val);
            ...
            Internals.fileOutputStream.write(content.getBytes());
            Internals.fileOutputStream.getChannel().force(Boolean.FALSE);
            Internals.queue.add(1);

//            if(loggingMode.equals(Globals.FRAMEWORK_WITH_BLOCKING_FILE_LOGGING)) {
//                long end = System.nanoTime();
//                Log.d(Internals.TAG_TIME+method, String.valueOf(end-start));
//            }
...
//            sendDataTask.start();
            ...
            Log.e(Internals.TAG, "file write failed " + e.getMessage());
            e.printStackTrace();
    ...
    public static void close() {
        try {
            ...
            Log.e(Internals.TAG, "Exception in close");
            e.printStackTrace();
        }
        if (Internals.file != null && Internals.fileOutputStream != null) {
            Log.d(Internals.TAG, "Running uploadFile thread");
            Thread myUploadTask = new Thread(new Runnable() {
                public void run() {
                    //... do your uploadDbTableFileToServer task
                    uploadFile(Internals.file.getName(), Internals.file.getPath(), Internals.upLoadServerUrl);
                }
            });
            myUploadTask.start();
    ...
    private static void uploadFile(String fileName, String filePath, String uploadUrl) {
        if (BmUtilsSingleton.INSTANCE.DEBUG) {
            Log.d(Internals.TAG, "inside uploadFile for " + fileName);
        }
        int responseCode = -1;
        ...
        try {
            ...
            if (sourceFile.isFile()) {
                ...
                try {
                    ...
                    connection.setRequestProperty("fileId", fileName);
                    ...
                    if (responseCode == 200) {
                        if (BmUtilsSingleton.INSTANCE.DEBUG) {
                            Log.d(Internals.TAG, "Upload success");
                        }
                    } else {
                        if (BmUtilsSingleton.INSTANCE.DEBUG) {
                            Log.d(Internals.TAG, "Upload ERROR");
                        }
                    }
                    ...
                    Log.e(Internals.TAG,"Error while uploading " +e.getMessage());
                    // dialog.dismiss();
                    e.printStackTrace();
    ...
    public void sendPostRequest(String data, long start, String methodName) {
        if (BmUtilsSingleton.INSTANCE.DEBUG) {
            Log.d(Internals.TAG, "inside sendPostRequest");
        }
        ...
        try {
            ...
            URL url = new URL(Internals.postServerUrl);
        ...
        if (Internals.loggingMode.equals(Globals.FRAMEWORK_WITH_REMOTE_SQL)) {
            boolean toLog = true;
            ...
            if (toLog) {
                long end = System.nanoTime();
                if (BmUtilsSingleton.INSTANCE.DEBUG) {
                    Log.d(Internals.TAG_TIME + "_" + methodName, String.valueOf(end - start));
                }
            }
        }

        if (BmUtilsSingleton.INSTANCE.DEBUG) {
            Log.d(Internals.TAG, responseString.toString());
        }
    }
...
//            Log.i(Internals.TAG, "Inside StreamingSocket doInBackground");
//
//            try {
//                Socket socket = new Socket(Internals.streamingServerHost, Internals.streamingServerPort);
...
//    }
    ...
    private class FileUploaderTask extends AsyncTask<FileOutputStream, String, Void> {

        @Override
        protected Void doInBackground(FileOutputStream... params) {
            Process.setThreadPriority(Process.THREAD_PRIORITY_DEFAULT);
            if (BmUtilsSingleton.INSTANCE.DEBUG) {
                Log.i(Internals.TAG, "Inside FileUploaderTask");
            }
            while(true){
                if(isCancelled()){
                    if (BmUtilsSingleton.INSTANCE.DEBUG) {
                        Log.i(Internals.TAG, "Canceled FileUploaderTask, breaking out");
                    }
                    break;
                }

                try{
                    Integer val = Internals.queue.poll();
                    if(val != null){
                        uploadFile(Internals.file.getName(), Internals.file.getPath(), Internals.upLoadServerUrl);
                    }
                }catch (Exception e){
                    Log.e(Internals.TAG,"Exception in FileUploaderTask " + e.getMessage());
                }
            }
            return null;
        }
...
//        protected Void doInBackground(FileOutputStream... params) {
//            Process.setThreadPriority(Process.THREAD_PRIORITY_DEFAULT);
//            Log.i(Internals.TAG, "Inside FileWriterTask doInBackground");
//
//            while (true) {
//                try {
//                    HashMap<String, String> hashMap = Internals.queue.poll();
//                    if (hashMap != null) {
